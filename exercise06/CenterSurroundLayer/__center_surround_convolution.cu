    ...
    for (int b = 0; b < N; b++){
        for (int c_o = 0; c_o < C_o; c_o++){
            ...
            for (int c_i = 0; c_i < C_i; c_i++)
                out += I[b][c_i][y_o+1][x_o+1]*w_c[c_i][c_o]+w_s[c_i][c_o]*S(I, b, c_i, y_o+1, x_o+1);
            O[b][c_o][y_o][x_o] = out+w_b[c_o];
        }
    }
}
    ...
    torch::PackedTensorAccessor32<scalar_t, 4, torch::RestrictPtrTraits> dL_dI)
{
    ...
    for (int b = 0; b < N; b++){
        for (int c_i = 0; c_i < C_i; c_i++){
            float out = 0.0;
            for (int c_o = 0; c_o < C_o; c_o++)
                out += dL_dO_padded[b][c_o][y_i+1][x_i+1]*w_c[c_i][c_o]+w_s[c_i][c_o]*S(dL_dO_padded, b, c_o, y_i+1, x_i+1);
            dL_dI[b][c_i][y_i][x_i] = out;
        }
    }
}
