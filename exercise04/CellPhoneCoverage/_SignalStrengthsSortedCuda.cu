...
static __global__ void resetBucketsKernel(Bucket* outputBuckets)
{
    const unsigned int bucketIdx = threadIdx.y * BucketsPerAxis + threadIdx.x; 
    outputBuckets[bucketIdx].startIndex = 0; 
    outputBuckets[bucketIdx].numElements = 0;  

}

static __global__ void inputHistogramKernel(const Position* inputPositions, int numInputPositions, 
                                            Bucket* outputBuckets)
{
    ...
    if (idx < numInputPositions)
    {
        ...
        // atomicAdd for histogram
        int bucketId = bucketY * BucketsPerAxis + bucketX; 
        atomicAdd(&outputBuckets[bucketId].numElements, 1); 
    }
}

static __global__ void prefixSumKernel(Bucket* outputBuckets)
{
    const unsigned int t = threadIdx.y * BucketsPerAxis * threadIdx.x; 
    const unsigned int nBuckets = BucketsPerAxis * BucketsPerAxis; 

    __shared__ int shared[BucketsPerAxis*BucketsPerAxis]; 

    // load Bucket.numElements to shared memory
    if (t < nBuckets) shared[t] = outputBuckets[t].numElements; 
    __syncthreads(); 

    // prefix sum computation
    for (unsigned int offset = 1; offset < nBuckets; offset *= 2)
    {
        int val = 0; 
        if(t >= offset && t < nBuckets)
            val = shared[t - offset]; 
    ...
    {
        if(t==0)
            outputBuckets[t].startIndex = 0; 
        else 
            outputBuckets[t].startIndex = shared[t-1]; 
    }

}

static __global__ void initBucketOffsetsKernel(const Bucket* outputBuckets, int* bucketOffsets)
{
    const unsigned int idx = threadIdx.y * BucketsPerAxis + threadIdx.x; 
    const unsigned int numBuckets = BucketsPerAxis * BucketsPerAxis; 

    if(idx < numBuckets)
        bucketOffsets[idx] = outputBuckets[idx].startIndex; 

}

static __global__ void populateOutputKernel(const Position* inputPositions, 
                                            int numInputPositions, 
                                            Position* outputPositions, 
                                            const Bucket* outputBuckets,
                                            int* bucketOffsets)
{
    const unsigned int idx = blockIdx.x * blockDim.x + threadIdx.x; 
    ...
    if (idx < numInputPositions)
    {
        ...
        // increments bucket-wise offset 
        int writeIdx = atomicAdd(&bucketOffsets[bucketId], 1);
        outputPositions[writeIdx] = pos;
    }
}
                                     ...
                                     CudaArray<Bucket>& cudaOutputPositionBuckets)
{
    ...
    dim3 bucketBlock(BucketsPerAxis, BucketsPerAxis, 1); 
    resetBucketsKernel<<<1, bucketBlock>>>(outputBuckets); 

    int threadsPerBlock = 256; 
    int numBlocks = (numInputPositions + threadsPerBlock - 1) / threadsPerBlock; 
    inputHistogramKernel<<<numBlocks, threadsPerBlock>>>(inputPositions, 
                                                        numInputPositions, 
                                                        outputBuckets);

    prefixSumKernel<<<1, bucketBlock>>>(outputBuckets);

 
    CudaArray<int> cudaBucketOffsets(numBuckets); 
    int* bucketOffsets =  cudaBucketOffsets.cudaArray();
    initBucketOffsetsKernel<<<1, bucketBlock>>>(outputBuckets, bucketOffsets);  

    populateOutputKernel<<<numBlocks, threadsPerBlock>>>(inputPositions, 
                                                        numInputPositions, 
                                                        outputPositions, 
                                                        outputBuckets, 
                                                        bucketOffsets);
